                            ...
                            <div class="card-body">
                                <div class="row mb-5">
                                    ...
                                    <div class="mt-4 col-xl-3 col-lg-3 col-md-6 col-sm-12">
                                        <h6>To:</h6>
                                        @if(isset($x))
                                        <div> <span class="badge badge-success mb-2">{{$x['0']->name}}</span> </div>
                                        <div>{{$x['0']->alamat}}</div>
                                        <div>Email: {{$x['0']->email}}</div>
                                        <div>Phone: {{$x['0']->nowa}}</div>
                                        @else
                                        <div> <span class="badge badge-success mb-2">{{$y['0']->name}}</span> </div>
                                        <div>{{$y['0']->alamat}}</div>
                                        <div>Email: {{$y['0']->email}}</div>
                                        <div>Phone: {{$y['0']->nowa}}</div>
                                        @endif
                                    </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                            @foreach($y as $sv)
                                            <tr>
                                                <td class="center">xxx{{$sv->id}}</td>
                                                <td class="left strong">Service {{$sv->type}}</td>
                                                <td class="left">{{$sv->code}}</td>
                                                <td class="right">IDR {{$sv->amount}}</td>
                                ...
                                <div class="row">
                                    ...
                                    <table class="table table-clear">
                                        <form action="{{route('sv_order.post', $total)}}" method="post" enctype="multipart/form-data">
                                            @csrf
                                            <tbody>
                                                <tr class="left">
                                        ...
                                        <table class="table table-clear">
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="right">
                                                        <div class="pt-0">
                                                        <button name="order" value="{{$y[0]->id}}" type="submit" class="btn btn-primary btn-lg" href="javascript:void(0)"><i class="mr-0"></i>Order</button>
                                                        </div>
                                                    </td>
                                                </tr>
                                                </form>
